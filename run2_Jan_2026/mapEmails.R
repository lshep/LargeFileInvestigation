...
extract_maintainers <- function(file_name) {
  pkglist <- read.delim(file_name)[,1]
  ...
  for (pkg in pkglist) {
    ...
    try({
      ...
      # Initialize dataframe to store maintainers for this package
      maintainer_df <- NULL
      
      if (!is.null(json_data$email)) {
        # Single maintainer at top level
  ...
  # Combine all into a long dataframe
